...
- **Limits arrive without warning.** On a subscription, you find out you hit the 5-hour limit when the agent stops mid-task. Vantage shows your quota live, forecasts whether your pace lasts until the reset, and warns you before you hit it.
...
- **Cost and limits, live.** Tokens, estimated cost at the official list prices of Anthropic, OpenAI and Google, and on a subscription the 5-hour and weekly quota with a forecast.
...
vantage run <agent>     # instead of the agent itself: claude, codex, copilot, opencode or pi
...
| --- | --- | --- | --- | --- | --- |
| Claude Code | `vantage run claude` | yes | 5-hour and weekly (subscription) | ask | yes |
| Codex | `vantage run codex` | yes | 5-hour and weekly (subscription) | block instead | yes |
...
- **Usage limits:** only subscriptions have them (Claude Pro or Max, ChatGPT plans), and their provider reports them with every response. An API key has no 5-hour or weekly window: you pay per token, so the cost and a cost budget (`--max-cost`) are what count. Copilot's monthly premium requests are not in its responses, so Vantage cannot show them.
...
```bash
vantage run <agent>                                  # the agent as usual, e.g. its interactive chat
vantage run <agent> -- <agent options>               # everything after -- goes to the agent
vantage run claude -- --model claude-opus-5-5        # for example: pick the model
vantage run codex -- exec "fix the failing test"     # or: one prompt, no chat
```

When the agent exits, Vantage sums up the session: requests, tokens, estimated cost, on a subscription your quota, and in a git repository the files that changed. Each session has an id (like `2026-09-25T13-01-30-311Z_4i4f`); `vantage sessions` lists them, and the commands below take it to look back at one.
...
<p align="center">
  <img alt="vantage watch: what the agent is doing, the 5-hour and weekly limits, the session's cost and every tool call" src="images/watch.png" width="90%">
</p>
...
While the agent's chat is open, Vantage never writes into its terminal. Alerts appear in the agent's own UI, right after the reply, as `Vantage: …` or `Vantage ALERT: …` (how exactly depends on the agent, see [Supported agents](#supported-agents)). `vantage watch` shows them live as well:

- a secret was sent to the API (see [Secret warnings](#secret-warnings))
- on a subscription: a quota window reached 90% (change the threshold with `VANTAGE_QUOTA_WARN`), or a limit was reached and requests are blocked
- your budget was reached
- an action type set to `warn` was used for the first time in this session

Each alert comes once. Alerts still waiting when the agent exits are printed then; when the agent runs a single prompt without its chat, they are printed right away.
...
- **Action types:** `read` (reading and searching files), `write` (editing and creating them, patches), `shell` (commands), `network` (web fetches and searches, MCP tools) and `other`. Each agent's tools are sorted into these by what they do, whatever the agent calls them: a tool that runs commands is `shell` in every agent.
...
The file lives in the project's `.vantage/` folder: commit it, and your whole team works with the same rules. For a single session, `VANTAGE_POLICY="shell:deny,network:ask" vantage run <agent>` overrides the action types. The rules are the same for every agent. `allow` never widens what the agent itself permits; Vantage can only make it stricter.
...
In a git repository, every session ends with a list of the files it changed. Edits you made yourself meanwhile are included, and the list says so. Later:
...
To keep the agent away from your working copy entirely, use `vantage run --isolate <agent>`. It then works in a separate git worktree on its own branch, `vantage/<id>`. At the end, Vantage commits the work to that branch and shows what changed; you decide what happens to it:
...
Agents forget everything between sessions. Project memory is a few Markdown files that Vantage gives to the agent at the start of every session, whichever agent it is:
...
vantage run --no-memory <agent>                       # a session without it
```
...
- **Claude models** at [Anthropic's price list](https://platform.claude.com/docs/en/about-claude/pricing), **GPT models** at [OpenAI's](https://developers.openai.com/api/docs/pricing), **Gemini models** at [Google's](https://ai.google.dev/gemini-api/docs/pricing), whichever agent calls them. The same model has the same price however an agent names it (`claude-sonnet-4.6`, `anthropic/claude-sonnet-4-6`).
...
- **On a subscription** you do not pay per token: the cost is what the same use would cost on the API, and the quota bars are what counts. **With an API key** the cost is what you pay.
...
- **"… not found":** set `VANTAGE_AGENT_PATH` to the agent's executable, for example `$env:VANTAGE_AGENT_PATH = "C:\path\to\agent.exe"` in PowerShell.
- **The agent cannot reach its API under Vantage:** Vantage forwards its requests to the provider's default address. If your plan or company uses another one (a gateway, a Copilot Enterprise address), set it with `VANTAGE_UPSTREAM`.
...
- **Numbers look wrong:** `VANTAGE_DEBUG=1 vantage run <agent>` logs what the API returns, including the format it was read as and the rate-limit headers.
